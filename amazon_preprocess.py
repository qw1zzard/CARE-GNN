...
def cal_rating(new_reviews):
    ...
    for i, u in enumerate(new_reviews):
        ...
        # feature [2-11]
        vector = [ratings.count(r + 1) for r in range(5)] + [
            ratings.count(r + 1) / len(ratings) for r in range(5)
        ]
        # feature [12-13]
        ...
        # feature [15-18]
        vector += (
            [np.median(ratings)] + [max(ratings)] + [min(ratings)] + [np.mean(ratings)]
        )
...
def build_graph(new_reviews):
    ...
    # user-product-user
    products = {
        u: [review['asin'] for review in reviews] for u, reviews in new_reviews.items()
    }
...
def build_features(new_reviews):
    ...
    summ_length = [
        sum(
            [
                len(review['summary']) if 'summary' in reviews[0].keys() else 0
...
if __name__ == '__main__':
    ...
    for u, total in all_reviews.items():
        ...
        helpful, votes = (
            sum([single['helpful'][0] for single in total]),
            sum([single['helpful'][1] for single in total]),
        )
        if votes >= 20:
