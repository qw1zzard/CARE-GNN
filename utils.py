...
def test_sage(test_cases, labels, model, batch_size):
    ...
    for iteration in range(test_batch_num):
        ...
        f1_gnn += f1_score(
            batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1), average='macro'
        )
        acc_gnn += accuracy_score(
            batch_label, gnn_prob.data.cpu().numpy().argmax(axis=1)
        )
...
def test_care(test_cases, labels, model, batch_size):
    ...
    for iteration in range(test_batch_num):
        ...
        batch_nodes = test_cases[i_start:i_end]
        batch_label = labels[i_start:i_end]
        gnn_prob, label_prob1 = model.to_prob(
            batch_nodes, batch_label, train_flag=False
        )
        ...
        f1_label1 += f1_score(
            batch_label, label_prob1.data.cpu().numpy().argmax(axis=1), average='macro'
        )
        acc_label1 += accuracy_score(
            batch_label, label_prob1.data.cpu().numpy().argmax(axis=1)
        )
